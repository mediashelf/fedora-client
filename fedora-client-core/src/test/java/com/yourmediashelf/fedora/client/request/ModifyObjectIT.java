...
public class ModifyObjectIT extends BaseFedoraRequestIT {
    ...
    public void testModifyObjectLabel() throws Exception {
        ...
        FedoraResponse response = modifyObject(testPid).label(modifiedLabel).logMessage("testModifyObjectLabel()").execute();
        assertEquals(200, response.getStatus());

        GetObjectProfileResponse getOP = getObjectProfile(testPid).format("xml").execute();
    ...
    public void testModifyObjectLabelWithXParam() throws Exception {
        ...
        FedoraResponse response = modifyObject(testPid).xParam("label", modifiedLabel).logMessage("testModifyObjectLabelWithXParam()").execute();
        assertEquals(200, response.getStatus());

        GetObjectProfileResponse getOP = getObjectProfile(testPid).xParam("format", "xml").execute();
    ...
    public void testModifyObjectState() throws Exception {
        FedoraResponse response = modifyObject(testPid).state("I").logMessage("testModifyObjectState()").execute();
        assertEquals(200, response.getStatus());
